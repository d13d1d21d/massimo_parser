...
translator = CSVTranslator(f"output/massimodutti{file_suffix}-products.csv")
...
if parsed_products:
    create_df(parsed_products, PREFIX, False).to_csv(
        f"output/massimodutti{file_suffix}-products.csv",
        sep=";",
        index=False,
        encoding="utf-8",
    ...
    create_df(parsed_products, PREFIX, True).to_csv(
        f"output/massimodutti{file_suffix}.csv",
        sep=";",
        index=False,
        encoding="utf-8",
    ...
    translator.add_new_columns(f"output/massimodutti{file_suffix}-products.temp")
    translated_rows = 0
